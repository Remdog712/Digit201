...
newStopWords = ["c", "mmmmm", "oz", "mealmaster", "tm", "tb"]
...
grimmTalesPath = os.path.join(workingDir, 'food')
...
def clean_doc(doc):
    try:
        ...
        text = open(doc, encoding='cp1252').read()
...
sorted_topics = sorted(topics[90], key=lambda x: x[1], reverse=True)
